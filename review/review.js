...
/* The folder the site sits in, worked out from where this tool is served.
   Handles /cvbs-2026/review/, /review/ on a custom domain, and localhost. */
...
/* --------------------------------------------------------------- markers  */
var MARKER_CSS =
  '.cvbsr-box{position:absolute;top:0;left:0;width:0;height:0;pointer-events:none;z-index:2147483000}' +
  '.cvbsr-pin{position:absolute;width:26px;height:26px;margin:-13px 0 0 -13px;border-radius:50% 50% 50% 4px;' +
  'transform:rotate(-45deg);background:#E0B341;border:2px solid #fff;box-shadow:0 3px 10px rgba(0,0,0,.35);' +
  'pointer-events:auto;cursor:pointer;display:grid;place-items:center;font:700 12px/1 Inter,sans-serif;color:#3a2a00}' +
  '.cvbsr-pin span{transform:rotate(45deg)}' +
  '.cvbsr-pin.done{background:#1B8A5A;color:#fff}' +
  '.cvbsr-pin.on{outline:3px solid #3BC9D9;outline-offset:2px}' +
  '.cvbsr-hl{position:absolute;pointer-events:none;border:2px solid #3BC9D9;background:rgba(59,201,217,.14);' +
  'border-radius:3px;transition:all .06s linear;z-index:2147482999}' +
  'html.cvbsr-pinning,html.cvbsr-pinning *{cursor:crosshair !important}';
...
function prepFrame() {
  ...
  if (!frameDoc.getElementById("cvbsr-style")) {
    var st = frameDoc.createElement("style");
    st.id = "cvbsr-style"; st.textContent = MARKER_CSS;
    frameDoc.head.appendChild(st);
  }
  markerBox = frameDoc.getElementById("cvbsr-box");
  if (!markerBox) {
    markerBox = frameDoc.createElement("div");
    markerBox.id = "cvbsr-box"; markerBox.className = "cvbsr-box";
...
function setPinning(on) {
  ...
  if (frameDoc) {
    frameDoc.documentElement.classList.toggle("cvbsr-pinning", on);
    var hl = frameDoc.getElementById("cvbsr-hl");
    if (!on && hl) hl.remove();
  }
}
...
function onFrameMove(e) {
  ...
  if (!el || el.classList && el.classList.contains("cvbsr-pin")) return;
  var hl = frameDoc.getElementById("cvbsr-hl");
  if (!hl) {
    hl = frameDoc.createElement("div"); hl.id = "cvbsr-hl"; hl.className = "cvbsr-hl";
...
function wire() {
  ...
  $("#btn-csv").addEventListener("click", function () {
    ...
    a.download = "cvbs-website-feedback.csv";
    a.click();
  });
