...
def flip_tone(section):
    """Swap the band tone on a section's opening tag, white for stone."""
    m = re.match(r'<section class="s-(white|stone) ', section)
    if not m:
        return section
    other = 'stone' if m.group(1) == 'white' else 'white'
    return section.replace('class="s-%s ' % m.group(1), 'class="s-%s ' % other, 1)
...
def build_full(city, d):
    ...
    # PARKED 7 Sep 2026. The "By delegate numbers" block came off the destination
    # pages at CVBS's request, to be picked up later. Run build_city_index.py for
    # a city and the block comes back; leave it unrun and this script now builds
    # the page without it instead of stopping. When it is absent the bands below
    # the featured section flip tone, so the white/stone rhythm still alternates
    # across the gap the block used to fill.
    ...
    below = [sec_start(city, d),
             ...
             sec_related(city, d)]
    if not index:
        below = [flip_tone(b) for b in below]
...
def build_patch(city, d):
    ...
    # Keep the white / stone band rhythm. Retoned 7 Sep 2026 when the venue index
    # came off the page: the block above trade-offs is now stone, so the tail runs
    # trade-offs (white) -> FAQ (stone) -> related (white) -> CTA. If the index is
    # ever republished on this page, this reverts to FAQ white and related stone.
    s = s.replace('<section class="s-stone pad" id="%s-tradeoffs"' % slug,
                  '<section class="s-white pad" id="%s-tradeoffs"' % slug, 1)
