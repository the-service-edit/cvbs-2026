...
.tdiv{grid-column:1/-1;font-size:10.5px;font-weight:700;letter-spacing:.14em;text-transform:uppercase;color:#5f7688;margin:9px 0 1px}
.tdiv.first{margin-top:0}
...
const COVERS=new Set(["offer","venue","event","educational","positioning","destination","roundup","result"]);
function tdiv(t,first){const d=document.createElement('div');d.className="tdiv"+(first?" first":"");d.textContent=t;return d;}
function buildTemplateButtons(){const c=document.getElementById('tmpls');c.innerHTML="";c.appendChild(tdiv("Covers",true));let split=false;
 Object.keys(T).forEach(k=>{
  if(!COVERS.has(k)&&!split){c.appendChild(tdiv("Content pages"));split=true;}
  const b=document.createElement('button');b.textContent=T[k].name.replace('› ','');b.className=k===cur().template?"on":"";
