...
| hilton.png      | Hilton                       | MISSING, row shows text         |

`hilton.jpeg` in this folder is the REVERSED lockup, white type on a navy box.
It is unusable on a white ground and nothing references it. Replace it with the
standard dark Hilton "Hilton Hotels & Resorts" lockup on a transparent ground,
saved as `hilton.png`. Until then the Hilton row renders as the word "Hilton",
which is correct, just plainer than the rest.

`accor.jpeg` and `tfe-hotels.jpeg` are the original sources. Nothing references
them. They can go whenever the folder is next tidied.
...
same pixel height, which is why the stacked marks run 62 to 66px and EVT, a wide
one-line lockup, runs 26px.
